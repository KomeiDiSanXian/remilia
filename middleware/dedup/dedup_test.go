...
func TestDedupMiddleware(t *testing.T) {
	t.Run("blocks_duplicate_events", func(t *testing.T) {
		...
		ctx1 := testutil.CreatePlatformContextWithID("event-1")
		err := wrappedHandler(ctx1)
		assert.NoError(t, err)
		assert.Equal(t, int32(1), handlerCalled.Load())

		// 第二次调用（重复事件 ID）
		ctx2 := testutil.CreatePlatformContextWithID("event-1")
	...
	t.Run("allows_different_events", func(t *testing.T) {
		...
		ctx2 := testutil.CreatePlatformContextWithID("event-2")
		err = wrappedHandler(ctx2)
		assert.NoError(t, err)
	...
	t.Run("continues_on_cache_full", func(t *testing.T) {
		...
		// 填满缓存
		for i := range 2 {
			_ = wrappedHandler(testutil.CreatePlatformContextWithID(fmt.Sprintf("event-%d", i)))
		}

		// 添加第3个事件（缓存满）
		ctx3 := testutil.CreatePlatformContextWithID("event-3")
	...
	t.Run("skips_events_without_id", func(t *testing.T) {
		...
		// 没有 ID 的事件
		ctx := testutil.CreatePlatformContextWithID("")
