...
func (e *Engine) registerMatcher(m *Matcher) *Matcher {
	...
	// 3. 添加到副本 (addMatcher handles indexing based on m.command)
...
//
// 相当于：engine.OnAny(OnFullMatch(text))
func (e *Engine) OnFullMatch(text string, rules ...Rule) *Matcher {
	finalRules := append([]Rule{OnFullMatch(text)}, rules...)
...
func (e *Engine) OnTemp(eventType dto.EventType, rules ...Rule) *Matcher {
	matcher := &Matcher{
		EventType:   eventType,
		Rules:       rules,
		coordinator: e,
...
//   - command: 触发命令 (如 "/ping")
//   - rules: 其他附加规则
func (e *Engine) OnCommand(eventType dto.EventType, command string, rules ...Rule) *Matcher {
	// 构造规则列表，首位为 standard OnCommand rule
	// 这样即使优化失效或降级，也能正确匹配
	finalRules := make([]Rule, 0, len(rules)+1)
	finalRules = append(finalRules, OnCommand(command))
	finalRules = append(finalRules, rules...)
...
func (e *Engine) RegisterCommandWithPrefix(prefix string, cmd *command.CommandDefinition, rules ...Rule) *Matcher {
	...
	parseRule := func(ctx *Context) bool {
		...
		if err != nil {
			logrus.WithError(err).WithField("trigger", trigger).Debug("[Engine] Command parse match failed")
			return false
		}
	...
	finalRules := append([]Rule{parseRule}, rules...)
	matcher := e.OnCommand(dto.EventType(""), trigger, finalRules...)
	matcher.Handle(ExecuteCommandDefinition)
	return matcher
}
