// embedding.go — Embedding 客户端、余弦相似度与文本向量缓存。
//
...
//
// baseURL 需为 API 根地址（如 https://api.openai.com/v1），/embeddings 会在其后
// 拼接，为空时返回 nil（不启用语义检索）。实现包含熔断器、维度漂移检测与
// 30s 超时，供工具选择、记忆检索与同仓其它插件（如知识库检索）复用。
