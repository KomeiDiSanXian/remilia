...
func TestStopCommandIdle(t *testing.T) {
	p := &Plugin{sm: session.NewSessionManager(100, 20, time.Hour, nil)}
...
func TestStopCommandCancelsActivePlan(t *testing.T) {
	...
	sess.SetPlan(&session.Plan{
		Task:   "查天气",
		Active: true,
		Steps:  []session.PlanStep{{ID: "step_1", Description: "查询城市天气", Status: session.PlanPending}},
	})
	if !sess.BeginTurn() {
	...
	if snap := sess.PlanSnapshot(); snap != nil && snap.Active {
...
func TestProcessWithToolsStopMidStreamSilentClose(t *testing.T) {
		...
		prov: &mockProvider{
			chatStreamFn: func(ctx context.Context, req *protocol.ChatRequest) (<-chan protocol.StreamEvent, error) {
				...
				go func() {
					defer close(ch)
					select {
					case ch <- protocol.StreamEvent{Type: protocol.StreamEventText, Content: "已经生成的部分"}:
					case <-ctx.Done():
						return
					}
	...
	sess := p.sm.GetOrCreate(sid, "user", "chat")
	p.sm.AppendMessage(sess, protocol.Message{Role: protocol.RoleUser, Content: "hi"})
	if !sess.BeginTurn() {
	...
	if len(msgs) != 2 || msgs[1].Role != protocol.RoleAssistant || msgs[1].Content != "已经生成的部分" {
	...
	p := &Plugin{
		...
		prov: &mockProvider{
			chatStreamFn: func(ctx context.Context, req *protocol.ChatRequest) (<-chan protocol.StreamEvent, error) {
				...
				go func() {
					defer close(ch)
					ch <- protocol.StreamEvent{Type: protocol.StreamEventText, Content: "半截回答"}
					<-ctx.Done()
					// 模拟 Anthropic 在连接被取消后以错误事件收尾。
					ch <- protocol.StreamEvent{Type: protocol.StreamEventError, Err: errors.New("context canceled")}
				}()
				return ch, nil
			},
		},
