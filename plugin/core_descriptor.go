...
type Descriptor struct {
	...
	// 适用于通过 [Service] 访问的强依赖。
	//
	// **依赖声明是插件正确性的必要契约**：加载顺序（拓扑排序）、循环依赖
	// 检测、依赖重载级联（OnDependencyReloaded）、级联注销都基于声明。
	// 所有通过 [Service]/mustGet 访问的插件依赖都应声明在此。
	// 未声明时框架仅对"批内依赖顺序错误"提供自动重试兜底；跨批顺序、
	// 重载级联等场景无法自动推断。
	Deps []string
	...
	// 适用于通过 [TryService] 访问的弱依赖（插件可在无该依赖的情况下正常运行）。
