...
2026-09-18 18:26 起，QQ 入群欢迎（`GROUP_MEMBER_ADD`）开始稳定失败：
`HTTP 400 code=40034025 message="请求参数event_id无效"`。`event_id` 本身是官方文档
认可的被动消息类型（「被动消息（响应事件）—— 对事件的回复」），因此这不是"该事件
不允许回复"。

- **根因**：平台把本次推送的事件**登记为"可回复"的时刻，比它把回调交到我们手里晚
  约 0.6~0.8 秒**。窗口内同一个 `event_id` 一律被拒，窗口一过立刻可用——"收到回调
  就回复"（`latency_ms=0`）必然踩空。实测（webhook 路径，7 条真实入群事件 / 19 次
  尝试）："仍未登记"的最晚时刻是收到回调后 `623ms`，"已登记"的最早时刻是 `771ms`，
  窗口落在 `(0.62s, 0.78s]`，无反例；被拒请求 120 ~ 218ms 就返回（只在请求早期做了
  一次校验），成功请求 480 ~ 771ms（真的投递）。窗口按**每次投递**重新计时——同一条
  事件被平台重投时窗口会重来一遍
- **已排除的解释**：写法错误（原样 `GROUP_MEMBER_ADD:<uuid>` 正确，裸 id 必被拒）、
  内容问题（纯文本 `123` 同样被拒）、"该事件不支持回复"（那会稳定返回 `40034027`）、
  "被拒即完成登记"（有一条连拒三次后第四次才成功）、"以事件生成时刻计时"（生成后
  4.65s / 12.30s 仍被拒，而生成后 3.71s 有成功），以及与本框架的 ACK 无关（生产回
  ACK 也照样被拒，且平台本身会概率重复投递）
- **修复**（`platform/qq/`，新增 `passive_eventid.go`）：首次尝试被拒且错误码为
  `40034025` 时，用**同一个 `event_id`** 按 `1.5s → 2s → 3s` 退避重试。首档取 1.5s
  是对实测上界 `0.771s` 留约 2 倍余量；初始 + 3 次重试用掉平台"每个事件最多被动回复
  5 次"配额中的 4 次，总耗时约 6.5 秒，远在群聊被动回复 5 分钟有效期之内。
  `40034026`（已过期）/`40034027`（该事件不支持回复）是确定性拒绝，直接返回；
  `msg_id` 相关错误码一律不重试（重试只会绕过平台的被动回复限制）
- **刻意不做主动消息兜底**：主动消息是另一种消息类型——不带 `event_id` 授权、需要
  群内开启「允许主动在群聊内发言」（未开启时 `40034105`，两个测试群均如此）、并另计
  主动消息配额。把插件作者写的"回复这个事件"静默换成"机器人主动发言"，等于替他改了
  语义与权限要求。平台既已认可 `event_id` 被动回复，被拒就应如实报错，由调用方决定
  是否改用主动消息（自行发一条不带 token 的请求即可）。这一条同时修正了此前"失败即
  清空 `event_id` 以主动消息重发"的做法
- **可观测**：每次重试失败记 DEBUG `[qq.Sender] event_id 被动回复重试仍未通过`（带
  `delay_ms`），首次成功记 WARN `[qq.Sender] event_id 被拒后重试成功`。于是一次入群
  就能从日志还原"事件到达后多久平台才开始接受该 `event_id`"。诊断用环境变量
  `QQ_PASSIVE_REPLY_DELAYS`（逗号分隔秒，最多 4 个）可覆盖重试间隔以提高分辨率，
  非法值 WARN 后回退默认
- **插件提示**（`builtin/welcome/`）：`/welcome set`、`/welcome global set` 的 QQ
  提示改为"框架会用同一个 `event_id` 重试；若仍失败会记录错误、不会改发主动消息"
  （顺带修正 `/welcome global set` 误显示"欢迎消息已设置"的问题，恢复为"全局欢迎
  消息已设置"）。退群告别 `GROUP_MEMBER_REMOVE` 行为不变：该事件确实不支持被动回复
  （`40034027`），只能以主动消息发送
- **回归测试**：`passive_eventid_test.go` 锁定重试次数与"不做主动消息兜底"
  （重试用尽只发 4 次、错误码仍为 `40034025`；`40034026`/`40034027` 只发一次；
  未带 `event_id`、`msg_id` 被拒、内容违规、主动消息频控等场景一律不重试），
  `QQ_PASSIVE_REPLY_DELAYS` 的解析与非法值回退；`webhook_eventid_window_live_test.go`
  是走真实分流 webhook 的时间窗探针（`//go:build network`，含自签合成事件的自检模式），
  可对平台行为复验
- **回归**：`gofmt` / `go fix` 无改动，`golangci-lint` 0 issues，`go vet -tags network`
  干净，`go build ./...` 与 `go test ./...` 全绿

## v1.64.0 (2026-09-17)

### ⚡ AI：把提示词前缀缓存做成可测量的工程目标

提供商侧的前缀缓存（DeepSeek 磁盘缓存、OpenAI / Anthropic prompt cache）只在请求
前缀逐字节一致时命中。而此前请求最前面的几段每轮都在变——System 消息里混着当前时间、
历史窗口每轮向右滑一条、工具列表顺序随机——于是整段对话历史在提供商侧被反复重新
计费。本次发布把请求的四个段落固定下来，并把命中量接进指标：

- **System 只放稳定内容**：框架提示词 + 自定义指令（`system_prompt` / 群策略）构成
  唯一的 System 消息，逐轮字节一致；运行时上下文（含当前时间）、群聊窗口、长期记忆、
  相关历史全部移出 System。上下文预算改为对着稳定提示词扣减，构建函数相应更名
- **动态上下文附在当前用户消息里**：每个回合只构建一次并复用，且不写回会话历史——
  上一轮的旧时间与旧群状态不会污染后续请求的前缀；也不新增消息（Anthropic 只接受
  顶级 system 字段，消息数组要求 user/assistant 交替）。同一回合的各次工具轮因此
  共享同一份字节一致的前缀，本回合产生的工具结果能继续被缓存复用
- **历史裁剪改为批量到低水位**：超过 `max_history` 时一次多裁若干条，而不是每轮滑
  一条——逐条滑窗会让历史前缀每轮右移，即使 System 稳定也永远命中不了
- **工具顺序确定**：工具注册表底层是 map，直接遍历的顺序每次调用都不同；`List()`
  改为按工具名升序返回，工具集不变时序列化结果字节稳定
- **执行计划移到消息序列末尾**：计划此前插在稳定前缀之后的中段，每步更新都要让其后的
  历史重新计费
- **缓存命中量可观测**：解析各提供商返回的缓存命中输入 token（OpenAI
  `prompt_tokens_details.cached_tokens`、DeepSeek `prompt_cache_hit_tokens`、Anthropic
  `cache_read_input_tokens`），导出为 `ai_llm_tokens_total{type="prompt_cached"}`；
  命中率 = `rate(prompt_cached)/rate(prompt)`，前缀结构的改动从此可以量化验证
- **不变量回归测试**：新增 `prompt_cache_test.go` 锁定 System 逐轮字节一致、动态上下文
  只出现在最后一条 user 消息且不落库、计划在末尾、工具顺序确定，以及相邻请求的公共
  前缀覆盖上一轮的全部历史

### 🧰 AI：工具集稳定（`tool_set_sticky`）

`tools` 是请求的顶级字段，在提示词序列里排在 system 之前，等于是整个请求的
**全局前缀开关**：工具集合一变，其后的稳定 System 与整段历史全部失去缓存。而工具
检索按当前 query 打分取 Top-K，话题一抖动就会换掉集合里的补充项，于是历史又被反复
打回未命中。本次在检索之上加一层会话级稳定策略：

- **子集保持**：候选是当前集合的子集（含相等）时不动——一轮没被选中不等于话题不再
  需要它，因此话题往复（A→B→A）与回访完全不产生集合变化
- **并集增长**：候选新增工具时取并集，只增不减，避免话题来回摆动时反复替换
- **双向回收**：补充工具超过 `tool_set_sticky_max`（默认 8）按最近使用时间淘汰，空闲
  超过 `tool_set_ttl`（默认 20 分钟）批量衰减；两者都是整批操作，回收时不会逐轮零敲
  碎打地改动集合
- **权限边界**：稳定集合始终是本轮可用工具（群策略白名单 + RBAC 过滤后）的子集，被
  收回的工具立即从集合与状态中剔除——工具集状态不会成为绕过过滤的旁路
- **配置与指标**：新增 `tool_set_sticky`（默认开启；设为 false 恢复「每轮按 query
  重新决定集合」，便于 A/B 对比缓存收益）、`tool_set_sticky_max`、`tool_set_ttl`；新增
  `ai_toolset_changes_total{reason}` 与 `ai_toolset_size`，抖动率（变更次数 / 请求次数）
  可与缓存命中率对照
- **代价**：每轮工具数量可多出至多 `tool_set_sticky_max` 个，且集合滑出 `tool_budget`
  时不再追加补充项（可调高 `tool_budget` 或调低上限）
- **回归测试**：`toolset_test.go` 锁定「集合何时允许变化」而非「选得准不准」——重复
  回合的字节稳定、话题切走后回访集合不变、超限按最近使用淘汰、TTL 衰减、权限收缩
  立即生效，以及 `tool_set_sticky=false` 时回到旧行为

### 🔧 构建、测试与 CI 稳定性

- **`infra/buildinfo` 测试顺序无关**：`TestGet_DefaultsToEmpty` 断言包级 `atomic.Value`
  从未被写入，但同文件其他用例会留下值，`-shuffle=on` 下默认值用例可能排到后面而失败；
  改为每个用例开头重置包状态
- **`builtin/ai` 指标测试改为断言增量**：三个用例此前断言包级 Prometheus 计数器的
  绝对值，而计数器是进程级只增不减的，`-count=N`（N>1）会因累加值失败；改为调用前后
  取差，与仓库其他指标测试一致
- **示例改用 `infra/pprof`**：`examples/showcase` 引用了已标记 `Deprecated` 的根包转发
  别名，触发 staticcheck SA1019；改为直接引用 `infra/pprof`
- **Windows CI 崩溃诊断**：测试任务此前遇到 `Exception 0xc0000005` 时只能看到故障
...
- **共用渲染层**：ISS 与 CSS 卡片收敛到 `cmd/bot/plugins/satutil` 的共用组件
  （主题、面板、环形仪表、状态徽章、图标、高度曲线、地面轨迹与轨道数学）。
  两个插件各自保留数据源与解析——CSS 走 CMSE 官方轨道预报，ISS 走
  wheretheiss.at 与 open-notify——只共享渲染与计算
- **卡片重绘**：两张卡片统一为深色玻璃拟态布局——分层渐变背景与顶部柔光、
  实时高度与速度、状态徽章、三块带环形仪表的数值面板（轨道周期 / 轨道速度 /
  可见范围）、地面轨迹图与高度曲线，配色分别为深空靛蓝与赤金暗红
- **真实海岸线**：内置 Natural Earth 1:110m 海岸线轮廓（134 条折线、5128 个
  顶点，约 20 KB，公有领域），以 1e-2 度量化后编译进二进制，运行时不访问任何
  外部服务；保留全部原始顶点、不做简化，避免大陆形状失真
- **投影严格对齐**：数据本身即 WGS84 经纬度，地图使用同一等经纬投影，海岸线按
  真实坐标落位而非伸缩拟合，因此与经纬网、星下点轨迹严格对齐；折线已在 ±180°
  处断开，不会出现横贯全图的假线段；绘图区锁定 2:1，大陆不被拉伸
- **ISS 地面轨迹**：接口只返回星下点与高度、不返回轨道倾角，故以 51.64° 常量按
  圆轨道近似推算单圈星下点轨迹（当前时刻前后各半圈）
- **在轨人数**：open-notify 的 `number` 统计的是全太空人数（含中国空间站乘组），
  卡片改为优先按筛出的 ISS 乘组名单计数，名单为空时才回退该字段
- **CSS 相位与趋势**：新增地影相位判定与当前相位剩余时长（在单圈窗口内扫描切换
  点，窗口内无切换则标记为未知）；近/远地点改用径向高度——大地高度含大地纬度
  引起的椭球起伏（41.5° 倾角上约 ±8 km），会把起伏误判成轨道变化；长期变化率
  改用相位对齐采样拟合，使一个轨道周期内的径向起伏相互抵消，只保留真正的抬升
  或衰减
- **降级不变**：图片渲染失败时仍回退到既有纯文本，字段与措辞保持一致
- **回归**：`coastline_test.go` 校验折线数、顶点数、坐标范围与 ±180° 断点，并以
  12 处真实海角（好望角、合恩角、卢因角、厄加勒斯角、约克角等）在 120 km 内反查
  渲染轮廓落点、以 3 个大洋中心点作反证；`groundtrack_test.go` 覆盖圆轨道推算、
  纬度不超倾角、坐标往返与非法输入；`card_test.go` 与两个插件的 `image_test.go`
  覆盖面板尺寸、曲线与抗锯齿、徽章换行、±180° 环绕与渲染产物

### 🐛 修复：御神签运势排版与塔罗标签

- **运势排版**：`各项运势` 原以最多 4 列的 Markdown 表格输出，项目数超过一行时
  会被切成多张表，而分块之间只隔一个换行，后一张表的分隔行（`:---:`）被并进前
  一张表当作正文原样显示出来（截图中「盖新居、搬家 / 结婚、交往、旅行」之后那
  一行）。项目名长短也悬殊（「遗失物」对「盖新居、搬家、嫁娶、旅行、交往等」），
  表格在窄屏上必然被挤压。改为每项一行的列表，与纯文本版的「名称：内容」一一
  对应
- **塔罗标签**：Markdown 路径把 `牌意`、`解读` 写成 `**牌意** 值`（空格），而纯
  文本路径写的是 `牌意：值`，两条路径不一致，因此只有 Markdown 消息缺冒号；
  现统一为 `**牌意**：值` 与 `**解读**：值`
- **牌阵引用块**：牌阵位置说明 `> …` 后未留空行，CommonMark 的惰性续行会把紧跟
  的牌意与解读一并并进引用块，整段被当作引文渲染；单张牌不产生引用行，故只在
  多张牌阵出现。已补空行
- **防御性转义**：`mdCell` 更名 `mdInline`，转义不再局限于表格竖线，另覆盖反斜
  杠、反引号、星号、下划线与方括号，避免日后修订数据源时排版错乱

## v1.61.0 (2026-09-12)

### 🎴 御神签与塔罗：内置真实素材与完整中文解签

- **素材编译进二进制**：塔罗 78 张公有领域 RWS 牌图（`assets/tarot`），浅草寺
  签纸 100 番 × 2 页扫描（`assets/omikuji`，MIT）。运行时不依赖外部图床，
  同时删除外部图片下载缓存与对应的健康探针
- **御神签改发签纸图**：`/omikuji` 发送两页签纸的合成图，番号、吉凶、漢詩与
  解签由签纸本身承载。此前自绘的等级、签文与幸运属性与真实签纸并不相符
  （例如第 1 番实际为大吉，却被标为大凶），已连同编造数据一并移除
- **签文数据集**：补齐 100 番的吉凶、漢詩、译意与分类运势，并与内置签纸扫描
  交叉核对——吉凶全部落在真实七档内（大吉 17、吉 35、末吉 6、半吉 5、小吉 5、
  末小吉 2、凶 30），抽查番号的吉凶与首句与签纸一致
- **塔罗牌意**：78 张牌补齐牌位与解读；大阿尔卡纳逐张撰写正文，小阿尔卡纳由
  牌组主管领域与牌位所处阶段组合而成
- **单图牌阵**：`/tarot` 由「每张牌一张图 + 综合文字」改为整副牌阵合成一张
  JPEG 并带位置标签，三张牌阵体积由约 1.2 MB 降至约 274 KB
- **图文同发与降级**：支持 Markdown 的平台随图发送 Markdown 解签，否则退化为
  纯文本；平台不支持图文同发时拆成两条，图片渲染失败时只发文字
- **AI 工具**：`draw_omikuji`、`draw_tarot` 直接返回真实签文与解读，运势技能的
  提示词同步更新，避免模型自行杜撰签文或吉凶
- **回归**：`omikuji_test.go`、`tarot_test.go` 覆盖数据集完整性、吉凶分布指纹、
  查表越界、两种排版、牌阵合成宽度、缺牌面降级与抽牌去重；`assets_test.go`
  覆盖 278 张素材可解码与两页逐点布局；`preview_test.go` 在设置
  `FORTUNE_PREVIEW_DIR` 时输出样例卡片
- **清理**：删除生产路径已不再调用的 `renderTarotCard`、`renderErrorCard`，
  死代码不再靠 `//nolint:unused` 压住

### ⛏️ Minecraft：RCON 服务器管理 `/mcadmin`

- **默认关闭**：在既有只读 `/mc` 查询之外，提供面向运营与群主的服务器控制台
- **服务器只来自配置**：每条配置含名称、地址、可选 `rcon_host`/`rcon_port`、
  可引用环境变量的密码，以及可选的会话绑定（把该条目限制在某个群内）
- **最小命令面**：只暴露 `list`/`status`/`say`/`whitelist`/`kick`/`save`
  白名单子命令，没有原始控制台透传，被攻陷或误操作的调用者无法触达任意命令
- **破坏性操作需确认**：关闭白名单时不使用 `--yes`，而是回复绑定调用者与会话的
  一次性短码——命令解析器会把后一个 token 当作该标志的值
- **授权 fail-closed**：接受配置的权限节点或 superadmin 角色；群主只能操作绑定
  本会话的服务器；权限服务缺失时拒绝而非放开，避免把控制台交给所有群成员
- **输入净化与审计**：玩家名、理由与广播文本在发送前净化，避免换行被当成第二条
  控制台命令；密码不回显、不记日志、不写入审计；每个用户发起的操作都记录服务器、
  命令与结果
- **实现**：RCON 客户端重组多包响应（朴素实现会截断或阻塞），按服务器串行化命令
  并池化复用连接、空闲回收
- **回归**：进程内假服务器覆盖认证失败、不可达、多包输出与畸形帧；管理层覆盖
  配置解析、授权矩阵、会话绑定、输入净化与确认流程
- **限制**：RCON 仅适用于 Java 版；QQ 群消息不携带群角色，群主路径仅适用于会
  填充该信息的平台，否则回退到权限节点

### 🛡️ Minecraft：查询加固（负缓存 / 冷却 / GS4）

- **负缓存**（`err_cache_ttl`）：失败查询短暂记忆，避免对离线服务器重复付出
  完整直连超时加 API 兜底；调用方取消不作为服务器离线信号，不缓存
- **查询冷却**：按会话、按用户的间隔只作用于查询路径，`/mc add`、`/mc rm`、
  `/mc list` 不受影响；回复告知剩余等待时间而非静默失败
- **`gs4_mode`**（auto/always/never）：auto 只在 SLP 玩家样本不完整时探测；
  always 也会为空服务器获取软件与插件列表；非法值回退 auto
- **`block_private_targets`**：可选的私网与回环地址探测拦截，默认关闭，
  因为自建服务器才是常见目标
- **多目标对比**：`/mc` 在一张卡片内以有界并发比较多个服务器，单个失败不掩盖
  其他目标
- **运行时探针**：注册 `system/dependencies/minecraft`，暴露缓存规模与生效查询
  配置，不发起网络请求
- **回归**：覆盖负缓存与冷却契约、GS4 模式归一化、报文解析边界，以及收藏管理器
  的 nil 接收者安全
...
- **结构化解析**（`platform/qq`）：合并转发（message_type=102）的扁平渲染
  文本解析为 SegmentForward 段。实测双形态支持（2026-09 报文核验）：
  直发形态（首行 `[标题]` + `=== 消息 N ===` 分块）与被引用形态（无标题行，
  记录包在单个条目内，`[关联消息]` + `--- 第N条 ---` 缩进树）；递归还原
  嵌套转发/嵌套引用/多行正文，`[附件N]` 行提取为统一附件（URL/尺寸/大小，
  文件名含空格安全）；解析失败退化为普通文本，不丢数据
- **统一类型与渲染**（`platform/forward.go`）：`ForwardRecord`/`ForwardNode`/
  Kind 常量与 Extra 键（`forward_nodes`/`quote_forward`）上移 platform 包，
  下游插件零平台依赖消费；`ForwardRecordText` 统一渲染（媒体只出占位符
  不泄漏时效 URL；单条 200 rune、每层 50 条、3 层嵌套深防膨胀），跨平台
  降级摘要（title/summary）同步填充
- **引用聊天记录可读**（103 引用 102）：被引用记录解析挂 reply 段
  `quote_forward`，空正文不再以整段扁平文本兜底（记录内 `/update now`、
  `/mc` 等命令文本不进入 Content，防误触发插件）；`QuotedImage` 新增记录
  载荷提取（递归嵌套，优先级位于归一化引用附件之后）
- **AI 对话接入**（builtin/ai）：
  - 私聊直发合并转发：渲染记录文本进入对话；**群聊直发不触发**——QQ 转发
    无法携带 @，群里不存在显式触发意图（自主发言群亦然），防刷屏；记录仍
    入群聊窗口上下文，讨论时引用该记录即可
  - 引用聊天记录：回复上下文注入记录全文（替换 `"[聊天记录]"` 占位符）
  - 视觉：直发记录内图片（含嵌套记录）走标准图片管线注入（缓存/SSRF/
    大小校验/内容嗅探），上限 `max_images_per_message` 超出截断而非整条
    拒绝，截断部分在渲染文本中以 `[图片]` 占位可感知
- **消息记录**（builtin/messagelog）：直发合并转发入记录时渲染结构化文本，
  消息窗口 / 回复链 / 统计等文本型下游可感知转发内容
...
- **QQ 指令按钮（type=2，不产生互动回调）**：AI Markdown 回复（QQ 单聊/群聊，
  频道除外）底部附带"重新生成"（`/ai retry`）与"清空会话"（`/ai reset`）
  按钮；计划消息（create_plan 推送与 `/ai plan` 状态回复）附带"查看计划"
  （`/ai plan`），回合进行中时另附"停止生成"（`/ai stop`）。点击不产生
  INTERACTION_CREATE，规避 QQ webhook 下回调按钮投递延迟导致的"请求第三方
  失败"（与 about 插件同一通道）
- **自动发送策略**：仅"重新生成/查看计划"在 QQ 单聊手机端（8983+，
  `Enter=true`）点击自动发送；"清空会话/停止生成"不自动发送（点击仅填入
  输入框，避免误触）；桌面端与群聊填入 `@bot <命令>` 由用户手动发送
- **可靠的停止生成**：`/ai stop`（文本命令或计划消息上的"停止生成"按钮）
  通过回合级中断上下文**立即取消进行中的 LLM 流**——单轮流（无工具调用）
  同样立即生效，无需等流自然结束；已生成部分保留为回复；停止时**一并取消
  进行中的任务计划**，半途中断后不再按旧计划在后续回合/后台自动推进
- **校验短路**：主动停止/被新消息抢占后不再送入回答校验器，避免额外 LLM
  调用与二次生成（`verify_enabled` 开启时同样生效）
- **清空会话语义统一**：`/ai reset`、QQ 客户端"清空会话"按钮与原生 type=14
  （CLEAR_SESSION）入口共用同一路径——生成中拒绝并节流提示（进行中回合持有
  会话指针），空闲时删除会话历史
- **防刷屏**：`/ai retry`、`/ai reset` 文本命令与按钮共用会话级触发冷却
  （3s 窗口），忙时提示按会话节流，重复点击/连发命令不再刷屏
- 新配置 `qq_regen_button` / `qq_clear_button` / `qq_plan_button`（默认
  true）；`markdown=false` 或纯文本/富媒体/频道消息不附加按钮

### 🚀 QQ 平台能力

- **消息反馈（type=13，AI 赞/踩）解析**：反馈事件会把被点消息 action_button
  的 `callback_data` 原样回传为 `button_data`，现以 `feedback_opt`
  （LIKE/DISLIKE）为内容，避免上层误判为按钮命令（如"重新生成"被赞/踩触发）
- **原生清空会话（type=14，CLEAR_SESSION）解析**：无按钮/反馈数据，平台层
  合成固定内容 `clear_session` 供插件分派（builtin/ai 据此清空会话）
- 互动 DTO 补齐 `FeedbackOpt` 字段并标注各互动类型（11-20）；`action_button`
  标注为实验性透传字段（公开消息按钮请使用 keyboard 指令按钮）

### ✨ about 插件

- `/about`（含 `/botinfo`）的"查看命令列表"按钮在 **QQ 单聊（C2C）手机端
  点击自动发送** `/help`（`Enter=true`）；群聊/桌面端仅填入输入框，其余平台
  保持原回调行为

### 🎨 aimage 插件

- **发送前压缩**：生成图超过 `send_max_bytes`（默认 5MB）/ `send_max_dimension`
  （默认 4096）上限时经 `infra/imagekit` 压缩后发送（与 pic / sauce 同一套
  逻辑），避免 QQ 等平台超限降级为文件类型；0 = 不限制
- **QQ 变体建议键盘**：单图生成完成后附带一条提示键盘（`qq_variant_keyboard`，
  默认 true，仅 QQ 生效）：点击把"同款重绘 / 水彩、赛博朋克风格变体 / 常用
  尺寸"的 `/aimage` 命令填入输入框（type=2，不产生回调）；多图场景跳过

### 📌 qqpanel 插件

- 面板/菜单自动构建排除**未声明 Description 的内部指令**（如 pluginctrl
  动态注册的 `/开启`、`/封禁` 等管理指令），避免空白描述项挤占 20/10 项上限；
  默认排除清单新增 `/logs`

### 🔧 工程

- 文档与示例配置同步：`AI_PLUGIN.md`（QQ 按钮接入与停止生成说明）、
  `APP_PLUGINS.md`（aimage 压缩/变体键盘、qqpanel 排除规则）、`FAQ.md`
  （QQ 按钮回调不稳定的已知问题与指令按钮规避方案）

## v1.50.1 (2026-09-02)

### 🐛 修复

- **`/ai`、`/kb` 命令缺少描述**：主命令定义补全 `Description`，`/help` 命令列表
  与 `/qqpanel setup` 生成的指令面板元素不再显示空描述（已创建的面板需重新
  执行 `/qqpanel setup` 才会带上新描述）
