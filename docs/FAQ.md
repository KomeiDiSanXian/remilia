...
已知问题：QQ webhook 模式下互动回调（type=1）可能不被投递；即便投递成功，事件延迟也可能挤占 3 秒回应窗口，客户端仍显示"请求第三方失败"。内置插件（如 about 的"查看命令列表"、ai 的"重新生成/清空会话"）在 QQ 上使用指令按钮（type=2，点击自动发送文本命令）规避；自研插件请避免依赖 QQ 互动回调。
