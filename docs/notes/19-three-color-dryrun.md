...
**第三方插件绝不参与探测执行**。`WithInferDeps` 批量注册只对显式声明
`Descriptor.DryRunSafe: true` 的插件执行探测 Setup；未声明的插件以声明依赖
（`Deps`/`OptionalDeps`）参与排序，Setup 在任何路径下只执行一次。

原因：Setup 是插件初始化的核心，要求其"无副作用或幂等"对第三方作者难以达成。
探测执行是插件作者显式选择的特权，而非框架默认行为。

## 解决方案：三色标记法（Tri-color Marking）

将 GC 的三色抽象映射到依赖推断（仅作用于 DryRunSafe 插件）：
...
| **White 初始集** | 全体对象 | 全体 DryRunSafe 插件 |
...
未声明 DryRunSafe 的插件在解析轮直接标记 black（无追踪信息），
不参与探测与依赖边发现。
