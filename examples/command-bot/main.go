...
/user - 用户信息`
	...
	bot.Engine().OnCommand("", "/user").Handle(func(ctx *eventctx.Context) error {
		ctx.Reply(platform.TextMessage(fmt.Sprintf("你的用户ID: %s", ctx.GetSenderInfo().ID)))
		return nil
	})
