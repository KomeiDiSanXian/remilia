...
type SearchResult struct {
	Source     string   // 主命中的搜索引擎："SauceNAO" / "IQDB" / "TraceMoe" / "AnimeTrace"；多引擎命中时以 "+" 拼接
...
//
//   - ImageURL：可公开访问的原始图片地址。部分引擎服务端自行抓图时依赖它。
//   - Data：本地下载并预处理后的图片字节。SauceNAO / IQDB / TraceMoe /
//     AnimeTrace 优先使用 Data 以 multipart 直传，不依赖图片 URL 在引擎侧的可达性。
