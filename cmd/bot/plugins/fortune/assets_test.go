...
func TestRenderCards(t *testing.T) {
	t.Run("omikuji_two_pages", func(t *testing.T) {
		...
		assertCardPNG(t, data, page0.Bounds().Dx()+page1.Bounds().Dx()+omikujiPageGap)
	})
	...
	t.Run("omikuji_single_page", func(t *testing.T) {
		...
		assertCardPNG(t, data, page0.Bounds().Dx())
	})
	...
	t.Run("tarot_reverse", func(t *testing.T) {
		...
		data, err := renderTarotCard(reading, decodeAsset(tarotAssetPath(reading.Card.NameShort)))
		if err != nil {
			t.Fatalf("塔罗渲染失败: %v", err)
		}
		assertCardPNG(t, data, tarotCardWidth)
	})

	t.Run("tarot_without_face", func(t *testing.T) {
		reading := &TarotReading{Card: *tarotDeck["wa01"]}
		data, err := renderTarotCard(reading, nil)
		if err != nil {
...
// TestRenderOmikujiCardLayout 确认两页并排且互不覆盖。
func TestRenderOmikujiCardLayout(t *testing.T) {
	page0 := decodeAsset(omikujiAssetPath(42, 0))
	page1 := decodeAsset(omikujiAssetPath(42, 1))
	...
	// 允许少量通道误差（8-bit 量纲）：WebP 解码与 draw 的颜色转换取整路径不同。
	const tolerance = 4 // 约 4/255
	maxDelta := uint32(0)
...
// assertCardPNG 校验渲染结果是尺寸合理的 PNG。
func assertCardPNG(t *testing.T, data []byte, wantWidth int) {
	t.Helper()
	img, err := png.Decode(bytes.NewReader(data))
	if err != nil {
		t.Fatalf("输出不是有效 PNG: %v", err)
	}
	if img.Bounds().Dx() != wantWidth {
