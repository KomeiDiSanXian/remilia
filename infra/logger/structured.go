...
func (l *StructuredLogger) WithContext(ctx *context.Context) *StructuredLogger {
	...
	if event != nil {
		// 事件 ID
		if event.ID != "" {
			fields[LogFieldEventID] = string(event.ID)
		}

		// 事件类型
		if event.Type != "" {
			fields[LogFieldEventType] = event.Type
		}
	}
	...
	// Matcher 信息
	matcherSource := ctx.GetMatcherSource()
	if matcherSource != "" {
		fields[LogFieldMatcher] = matcherSource
	}
...
func (l *StructuredLogger) WithMatcher(matcher *engine.Matcher) *StructuredLogger {
	if matcher == nil {
		return l
	}
